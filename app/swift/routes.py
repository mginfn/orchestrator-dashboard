...
swift_bp = Blueprint(
    "swift_bp", __name__, template_folder="templates", static_folder="static"
)
...
def createswifttoken():
    if request.method == "POST":
        ...
        swift_v = (
            form_data["swiftauthversion"] if "swiftauthversion" in form_data else None
        )
...
def gets3creds():
    ...
    if request.method == "POST":
        ...
        project = next(
            (m["project"] for m in s3params["mapping"] if m["group"] == group), None
        )

        if not project:
            app.logger.error(
                "Get S3 creds error: project for group {} not found".format(group)
            )
            flash(
                " Sorry, something went wrong while getting S3 credentials: project {} not found".format(
                    group
                ),
                "danger",
            )
        ...
        access, secret = openstack.get_or_create_ec2_creds(
            access_token,
            project,
            auth_url,
        ...
        if access and secret:
            ...
            try:
                ...
                app.logger.error(
                    "Error while testing S3 credentials for user {}, group {}: {}".format(
                        session["username"], project, e
                    )
                )
                flash(
                ...
                utils.send_authorization_request_email(
                    "S3 credentials for project {}".format(project)
                )
                return render_template("s3creds.html", s3urls=urls)

            return render_template(
                "s3creds.html", s3creds=dict(access_key=access, secret_key=secret)
            )
        else:
            app.logger.error(
                "Null S3 credentials returned for user {}, group {}".format(
                    session["username"], project
                )
            )
            flash(
                " Sorry, something went wrong while getting S3 credentials for project {}".format(
                    group
                ),
                "danger",
            )
