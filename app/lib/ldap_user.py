...
class AlreadyExistingError(Error):
    def __init__(self, username, email):
        message = "Cannot retrieve stored info for existing ldap user {} with email {}".format(
            username, email
        )
...
class PasswordMismatch(Error):
    def __init__(self, username, email):
        message = "Password changed for existing ldap user {} with email {}".format(
            username, email
        )
        self.message = message
        super().__init__(message)
...
class LdapUserManager(object):
    ...
    def create_user(self, username, email):
        ...
        if username.encode("UTF-8") in existingusers:
            ...
            self.vault.v1_write_secret(
                "ldap_user", {"username": username, "password": pw}
            )
