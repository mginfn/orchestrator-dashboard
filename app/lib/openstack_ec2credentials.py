...
def get_unscoped_keystone_token(access_token, auth_url, identity_provider='infn-cc', protocol='oidc'):
    ...
    url = auth_url + '/v3/OS-FEDERATION/identity_providers/{}/protocols/{}/auth'.format(identity_provider, protocol)
...
def list_ec2_credentials(auth_url, user_id, project_id, scoped_token):
    ...
    if response.ok:
        ...
        credentials = [creds for creds in ec2_creds if creds.get('tenant_id') == project_id]
...
def get_or_create_ec2_creds(access_token, auth_url, identity_provider='infn-cc', protocol='oidc'):
    ...
    unscoped_token, user_id = get_unscoped_keystone_token(access_token, auth_url, identity_provider, protocol)
